...
public class RequestData {
	...
	public static String queryAllbids(Map<String, String> parameters) throws IOException{
		...
		String loanSchedule = parameters.get("loanSchedule");
		String startDate = parameters.get("startDate");
		String endDate = parameters.get("endDate");
		String loanType = parameters.get("loanType");
		String minLevelStr = parameters.get("minLevelStr");
		String maxLevelStr = parameters.get("maxLevelStr");
		String orderType = parameters.get("orderType");
		String keywords = parameters.get("keywords");
		
		
		PageBean<v_front_all_bids> bids = Invest.queryAllBids(currPage, pageSize, apr, amount, loanSchedule, startDate, endDate, loanType, minLevelStr, maxLevelStr, orderType, keywords, error);
	...
	public static String bidDetail(Map<String, String> parameters){
		...
		jsonMap.put("borrowtype", bid.product.nameImageFilename);//图片
	...
	public static String queryAllQuestions(Map<String, String> parameters){
		...
		map.put("questionList", page.page);
		map.put("error", -1);
		map.put("msg", "查询成功");
	...
	public static String queryAllDebts(Map<String, String> parameters){
		...
		String apr = parameters.get("apr");
		String debtAmount = parameters.get("debtAmount");
		String loanType = parameters.get("loanType");
		String orderType = parameters.get("orderType");
		String keywords = parameters.get("keywords");
		...
		if(error.code < 0){
			jsonMap.put("error", 1);
			jsonMap.put("msg", "对不起！系统异常，给您带来的不便敬请谅解！");
	...
	public static String auction(Map<String, String> parameters){
		...
		Debt.auctionDebt(userId, amount, debtId, error);
...
//		long userId = Long.(userIdStr);
	...
	public static String historicalRepayment(Map<String, String> parameters){
		...
		if(totle > 0){
			for(int i = 0;i < totle;i++){
				...
				map.put("repayAmount", bill.receive_corpus + bill.receive_interest);
	...
	public static String transact(Map<String, String> parameters){
		...
		Debt.dealDebtTransfer(null,debtId, error);
	...
	public static String queryAuctionRecords(Map<String, String> parameters){
		...
		ErrorInfo error = new ErrorInfo();
		Map<String,Object> jsonMap = new HashMap<String, Object>();
		String creditorIdStr = parameters.get("creditorId");
		...
		long debtId = Long.parseLong(creditorIdStr);
	...
	public static String increaseAuction(Map<String, String> parameters){
		...
		Debt.auctionDebt(userId, offerPrice, debtId, error);
	...
	public static String investStatistics(Map<String, String> parameters){
		...
		List<v_bill_invest_statistics> statistic = GenericModel.find(" user_id = ?", userId).fetch(1, 100);
	...
	public static String attentionBids(Map<String, String> parameters){
		...
		if(StringUtils.isBlank(keywords)){
			page.totalCount = (int) GenericModel.count("  user_id = ?  ",userId);
			page.page = GenericModel.find("  user_id = ?  ",userId).fetch(currPage,pageSize);
		}else{
			page.totalCount = (int) GenericModel.count("  user_id = ? and  title like ?",userId,"%"+keywords+"%");
			page.page = GenericModel.find("  user_id = ? and  title like ?",userId,"%"+keywords+"%").fetch(currPage,pageSize);
		}
		jsonMap.put("error",-1);
		jsonMap.put("msg", "查询成功");
		jsonMap.put("totalNum",page.totalCount);
	...
	public static String attentionUser(Map<String, String> parameters){
		...
		User.attentionUser(userId, attentionUserId, error);
	...
	public static String collectBid(Map<String, String> parameters){
		...
		Bid.collectBid(userId, bidId, error);
	...
	public static String collectDebt(Map<String, String> parameters){
		...
		Debt.collectDebt(userId, debtId, error);
	...
	public static String managementTeam(Map<String, String> parameters){
		...
		String content = News.queryContent(-1005, error);
	...
	public static String expertAdvisor(Map<String, String> parameters){
		...
		String content = News.queryContent(-1006, error);
	...
	public static String appVersion(Map<String, String> parameters){
		Map<String,Object> jsonMap = new HashMap<String, Object>();
		t_system_options option = null;
		
		try {
			option = GenericModel.findById(7006l);
		} catch (Exception e) {
			e.printStackTrace();
		...
		if(null != option){
			jsonMap.put("version", option._value);
		}
	 ...
	 * @return
	 */
	public static List<v_front_all_bids> queryFullBid(ErrorInfo error){
		List<v_front_all_bids> bids = new ArrayList<v_front_all_bids>();
		
		try {
			bids = GenericModel.find(" status in (1,2) and amount = has_invested_amount order by time desc ").fetch(3);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String home(Map<String, String> parameters){
		...
		List<v_bids> qualityBids = Bid.queryQualityBid(3, error);//三个优质借款标
		...
		List<Bid> investInfo = Bid.queryAdvertisement(error); // 最新投资资讯
		...
		List<v_front_all_bids> fullBids = queryFullBid(error); // 最新满标借款标
		...
		jsonMap.put("investInfo",investInfo);
...
//		HttpServletResponse response = ServletActionContext.getResponse();
	...
	public static String queryBaseInfo(Map<String, String> parameters) throws IOException{
		...
		List<t_dict_cars> cars = GenericModel.findAll();
		List<t_dict_ad_provinces> provinces = GenericModel.findAll();
		List<t_dict_educations> educations = GenericModel.findAll();
		List<t_dict_houses> houses = GenericModel.findAll();
		List<t_dict_maritals> maritals = GenericModel.findAll();
		List<t_dict_ad_citys> cityList = GenericModel.findAll();
	...
	public static String register(Map<String, String> parameters) throws IOException{
		...
		String password = parameters.get("pwd");
		String referrerName = parameters.get("referrerName");
		...
		if(StringUtils.isNotBlank(referrerName)) {
			recoName = Encrypt.decrypt3DES(referrerName, Constants.ENCRYPTION_KEY);
...
//		List<Object> listOrder = new ArrayList<Object>();
		...
		if(StringUtils.isBlank(parameters.get("yearRate"))){
			jsonMap.put("error", "-3");
			jsonMap.put("msg", "年利率有误");
		...
		if(StringUtils.isBlank(parameters.get("borrowTime"))){
			jsonMap.put("error", "-3");
			jsonMap.put("msg", "借款总额有误");
	...
	public static String productInfo(Map<String, String> parameters) throws IOException{
		...
		jsonMap.put("limitRange", Double.toString(product.minAmount)+"-"+ Double.toString(product.maxAmount));
...
//		}
	...
	public static String TwoDimensionalCode(Map<String, String> parameters) throws IOException{
		...
		jsonMap.put("promoteImg", "/images?uuid="+user.qrcode);
	...
	public static String queryMyLoanBills(Map<String, String> parameters) throws IOException{
		...
		PageBean<v_bill_loan> page = Bill.queryMyLoanBills(userId, 0, 0, 0, "", Integer.valueOf(parameters.get("currPage")), Constants.APP_PAGESIZE, error);
	...
	public static String submitRepayment(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("dealPwd"))){
			jsonMap.put("error", "-3");
		...
		User user = new User();
		user.id = userId;
		int code = user.verifyPayPassword(parameters.get("dealPwd"), error);
...
//		if(StringUtils.isBlank(parameters.get("startTime"))){
//			jsonMap.put("error", "1");
...
//		}
        ...
        PageBean<v_user_details> page = User.queryUserDetails(userId, Long.valueOf(parameters.get("purpose"))
        		,DateUtil.strToDate(parameters.get("startTime")), DateUtil.strToDate(parameters.get("lastTime")),Integer.valueOf(parameters.get("currPage")), Constants.APP_PAGESIZE);
	...
	public static String verifySafeQuestion(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("answer1"))){
			jsonMap.put("error", "-3");
		...
		if(StringUtils.isBlank(parameters.get("answer2"))){
			jsonMap.put("error", "-3");
		...
		if(StringUtils.isBlank(parameters.get("answer3"))){
			jsonMap.put("error", "-3");
		...
		User  user = new User();
		user.id = userId;
		user.verifySafeQuestion(parameters.get("answer1"), parameters.get("answer2"), parameters.get("answer3"), error);
	...
	public static String savePayPassword(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("newdealpwd"))){
			jsonMap.put("error", "-3");
			jsonMap.put("msg", "新密码有误");
		...
		user.addPayPassword(true, parameters.get("newdealpwd"), parameters.get("newdealpwd"), error);
	...
	public static String editPayPassword(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("currentdealpwd"))){
			jsonMap.put("error", "-3");
		...
		user.editPayPassword(parameters.get("currentdealpwd"), parameters.get("newdealpwd"),parameters.get("newdealpwd"),error);
	...
	public static String savePassword(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("newloginpwd"))){
			jsonMap.put("error", "-3");
		...
		if(StringUtils.isBlank(parameters.get("oldloginpwd"))){
			jsonMap.put("error", "-3");
        ...
        user.editPassword(parameters.get("oldloginpwd"), parameters.get("newloginpwd"),parameters.get("newloginpwd"),error);
	...
	public static String saveSafeQuestion(Map<String, String> parameters) throws IOException{
		...
		user.answer1 = parameters.get("answer1");
		user.answer2 = parameters.get("answer2");
		user.answer3 = parameters.get("answer3");
	...
	public static String saveEmail(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("emailaddress"))){
			jsonMap.put("error", "-3");
		...
		User  user = new User();
		user.id = userId;
		user.email = parameters.get("emailaddress");
		...
		String emailUrl = EmailUtil.emailUrl(parameters.get("emailaddress"));
	...
	public static String applyForOverBorrow(Map<String, String> parameters) throws IOException{
		...
		OverBorrow.applyFor(user.id, Integer.valueOf(parameters.get("excessAmount")), parameters.get("applyReason"), auditItems, error);
	...
	public static String aprCalculator(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("deadline")) || Integer.valueOf(parameters.get("deadline")) < 0){
			jsonMap.put("error", "-3");
			jsonMap.put("msg", "借款期限有误");
		...
		double serviceFee  = ServiceFee.investServiceFee(amount, Double.parseDouble(parameters.get("apr")), error); // 服务费
	...
	public static String deleteBank(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("accountId"))){
			jsonMap.put("error", "-3");
			jsonMap.put("msg", "银行卡有误");
			
			return JSONUtils.printObject(jsonMap);
		}
		
		UserBankAccounts.deleteUserBankAccount(Long.parseLong(parameters.get("accountId")), error);
	...
	public static String submitWithdrawal(Map<String, String> parameters) throws IOException{
		...
		if(StringUtils.isBlank(parameters.get("payPassword"))){
			jsonMap.put("error", "-3");
			jsonMap.put("msg", "交易密码有误");
		...
		user.withdrawal(Double.parseDouble(parameters.get("amount")), Integer.parseInt(parameters.get("bankId")), parameters.get("payPassword"), Integer.parseInt(parameters.get("type")), false, error);
