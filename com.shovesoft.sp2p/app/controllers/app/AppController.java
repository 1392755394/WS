...
////		Map<String, String> parameters = request.params.data;
//		 request.params.put("_t", Convert.dateToStr(new Date(), "yyyy-MM-dd HH:mm:ss", "1970-01-01 00:00:00"));
...
//		parameters.put("OPT", "1");
//		parameters.put("name", "18626850291");
//		parameters.put("pwd", "wangsong123");
		Http.Request requests = Http.Request.current();
		Set<String> keySet = requests.params.data.keySet();
		for (String a:keySet) {
			parameters.put(a, requests.params.get(a));
		}
		app.delegateHandleRequest(parameters, errorDescription);
//		String finalURL = GeneralRestGateway.buildUrl(request.url, Constants.APP_ENCRYPTION_KEY, parameters);
//		String finalURL;
//		try {
//			finalURL = GeneralRestGateway.buildUrl(request.url, Constants.APP_ENCRYPTION_KEY, parameters);
//			URL url;
//			try {
...
//			e.printStackTrace();
//		}
//		int a = GeneralRestGateway.handle(Constants.APP_ENCRYPTION_KEY, 3000, app, errorDescription);
