...
public class ProductAction extends SupervisorController {
	...
	private static boolean checkProduct(Product product, long productId){
		...
		for (String str : arr) {
			...
			if (value > 5 * 12 * 30 || value <= 0) {
				flash.error("日期限需在1~1800日之间!");
		...
		if (StringUtils.isBlank(product.smallImageFilename) || Constants.DEFAULT_IMAGE.equals(product.smallImageFilename)) {
			flash.error("借款小图标有误!");
			
			return true;
		}
		
		if (StringUtils.isBlank(product.nameImageFilename) || Constants.DEFAULT_IMAGE.equals(product.nameImageFilename)) {
		...
		if ((Constants.PLATFORM_UPLOAD == product.loanImageType && (StringUtils.isBlank(product.loanImageFilename))  || Constants.DEFAULT_IMAGE.equals(product.loanImageFilename))) {
			flash.error("借款图片有误!");
		...
		if(loanTypes.contains(product.loanType)){
		...
		if (product.minInterestRate <= 0 || product.minInterestRate >= 100) {
			flash.error("最低借款利率有误!");
			
			return true;
		}
		
		if (product.maxInterestRate > 100 || product.maxInterestRate <= 0) {
	...
	public static void addingAuditItem(AuditItem item){
		...
		if (item != null && Constants.IPS_ENABLE) {
			item.auditFee = 0;
		}
