...
public class PlatformAccountManager extends SupervisorController {
	...
	public static void withdrawReview(long withdrawalId) {
		...
		ErrorInfo error = new ErrorInfo();
		User.auditWithdrawalDispass(1L, withdrawalId, reason, error);
		flash.error(error.msg);
		toReviewWithdraws();
	}
	...
	public static void payNotification(long userId, long withdrawalId, String type) {
		...
		for (int i = 0; i < arr.length; i++) {
			User.withdrawalNotice(userId, withdrawalId, Convert.strToInt(arr[i], -1), false, error); 
		}
	 ...
	 * 充值记录
	 */
	public static void rechargeRecord(int type, int status, String name, Date startDate, Date endDate, int currPage){
	...
	public static void transactionRecords(int type, int operation, int side,
			Date beginTime, Date endTime, String name, int currPage) {
        ErrorInfo error = new ErrorInfo();
		...
		List<t_platform_detail_types> allType = DealDetail.queryType(error);
	...
	public static void offSingleDeal(String payNumber){
		...
		user_recharge = GenericModel.find("pay_number = ? and is_completed = ?", payNumber, false).first();
	...
	public static void ipsOffSingleDeal(String billno, String mercode, String Currency_type, String amount, String date, String succ,
		...
		t_user_recharge_details user_recharge = null;
		user_recharge = GenericModel.find("pay_number = ? and is_completed = ?", billno, false).first();
		if(null == user_recharge){
			renderText(info);
			return;
		}
	 ...
	 * 
	 * ***/
	public static void exportRechargeRecord(){
		
		ErrorInfo error = new ErrorInfo();

		List<t_user_recharge_details>  details = null;
	
		String hql = " from t_user_recharge_details where is_completed = true order by id desc";
		try {
			 details = GenericModel.find(hql).fetch();
		...
		File file = ExcelUtils.export("充值记录", arrDetails, new String[] { "用户",
				"时间", "支付网关", "支付流水号", "充值金额", "是否已经完成", "完成时间", "订单id", "充值类型"},
				new String[] { "name", "time", "payment_gateway_id", "pay_number",
						"amount", "is_completed", "completed_time", "order_no","type" });
	 ...
	 * 
	 * ***/
	public static void exportTransactionRecords(){
		
		ErrorInfo error = new ErrorInfo();

		List<v_platform_detail>  details = null;
	
		String hql = " from v_platform_detail  order by id desc";
		try {
			 details = GenericModel.find(hql).fetch();
		...
		jsonConfig.registerJsonValueProcessor(Date.class,
				new JsonDateValueProcessor("yyyy-MM-dd"));
		JSONArray arrDetails = JSONArray.fromObject(details, jsonConfig);
		//System.out.print(arrDetails);
		File file = ExcelUtils.export("交易记录", arrDetails, new String[] { "科目",
				"金额", "支付方式", "付款方", "收款方", "备注", "操作时间", "类型"},
				new String[] { "name", "amount", "payment", "from_pay",
						"to_receive", "summary", "time", "type"});
