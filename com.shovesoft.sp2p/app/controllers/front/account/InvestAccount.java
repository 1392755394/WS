...
public class InvestAccount extends BaseController {
	 ...
	 * @param paramter
	 */
	public static void queryInvestRecords(int pageNum,int pageSize,String type,String paramter){
	 ...
	 * @param debtId
	 */
	public static void increaseAuction(long debtId,String offerPriceStr) {
	    ...
	    Debt.auctionDebt(user.id, offerPrice, debtId, error);
	 ...
	 * @param debtId
	 */
	public static void transact(String sign){
		...
		if (Constants.IPS_ENABLE) {
			...
			Map<String, Object> args = Payment.registerCretansfer(pMerBillNo, fromUserId, toUserId, bidNo, pCreMerBillNo, pCretAmt, pPayAmt, managefee);
	 ...
	 * @param pageNum
	 */
	public static void queryUserAllloaningInvestBids(int pageNum,int pageSize,String type,String paramter){
	 ...
	 * 接收定向债权转让
	 */
	public static void acceptDebts(String sign){
	 ...
	 * 进入已完成的理财标页面
	 */
	public static void successInvestBid(){
		render();
	}
	 ...
	 * @param pageNum
	 */
	public static void queryUserSuccessInvestBids(int pageNum,int pageSize,String type,String paramter){
		...
		User user = User.currUser();
		PageBean<v_user_success_invest_bids> page = Invest.queryUserSuccessInvestBids(user.id, type, paramter, currPage, pageSize,error);
	...
	public static void removeBlacklist(long blacklistId) {
		...
		User user = User.currUser();
		user.deleteBlacklist(blacklistId, error);
	...
	public static void investStatistics(int year, int month, int orderType, int currPage){
		...
		PageBean<v_bill_invest_statistics> page = User.queryUserInvestStatistics(user.id, 
...
public static void loanAgreement(String sign){
	...
	Invest invest = new Invest();
	invest.bid = b;
	List<t_invests> investDetails=Invest.queryLoanAgreeAmount(bidId, useri.id, error);
	
    double totleAmount = 0;
	
    for(t_invests tot : investDetails){
    	   
    	totleAmount=totleAmount+tot.amount; 
	...
	List<t_bill_invests> tbilist = BillInvests.queryMyInvestBillReceivablesBid(bidId, useri.id, error);
	
	int dateDay = 0;
	if(b.repayment.id == 1){//按月还款、等额本息 
		for (t_bill_invests tBillInvests : tbilist) {
			tBillInvests.dxreceive_amount = new CnUpperCaser(tBillInvests.receive_amount+"").getCnString();
		}
	...
	//数字转大写
	String dxMoney = new CnUpperCaser(invest.bid.amount+"").getCnString();
	//b.period
	...
	render(noid,useri,b,userb,invest,dxMoney,dates,tbilist,dateDay,investDetails,totleAmount);
}
