...
public class UserBase implements Serializable{
	...
	public long login(String password){
		/*获取锁定的错误次数*/
		t_system_options option = GenericModel.find("byKey", OptionKeys.SECURITY_IS_USERNAME_LIMIT_WORDS).first();
...
//			this.errorInfo = "你已经被管理员禁止登录";
		...
		if(this.isPasswordErrorLocked) {
			t_system_options lockTime = GenericModel.find("byKey", OptionKeys.SECURITY_LOCK_TIME).first();
...
//				this.errorInfo = "用户已经锁定";
		...
		t_users user = GenericModel.findById(this.id);
		/*密码判断*/
...
//			this.errorInfo = "密码与用户名不匹配";
	...
	public long  logout(){
		Response.current().setCookie(this.getClass().getSimpleName(), "");
		return 0;
...
//		String cookie = id+"hello";
		...
		Response.current().setCookie(cookieKey, cookie);
	}
