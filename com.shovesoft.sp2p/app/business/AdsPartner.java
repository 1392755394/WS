...
public class AdsPartner implements Serializable{
	...
	public void setId(long id) {
		...
		try {
			partner = GenericModel.findById(id);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int createPartner(long supervisorId, ErrorInfo error) {
		...
		try {
			count = (int)GenericModel.count("_order = ?", this.order);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static t_content_advertisements_partner queryPartnerForId(
		...
		try {
			security = GenericModel.findById(adsId);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<AdsPartner> qureyPartner(String key, ErrorInfo error) {
		...
		try {
			if(StringUtils.isBlank(key)) {
				advertisements = GenericModel.findAll();
			}else{
				advertisements = GenericModel.find("name like ?","%"+key+"%").fetch();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<AdsPartner> qureyPartnerForFront(ErrorInfo error) {
		...
		try {
			advertisements = GenericModel.find(sql).fetch(
					1, 12);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("查找合作伙伴，查找合作伙伴信息时：" + e.getMessage());
	...
	public static boolean orderExist(String orderStr, ErrorInfo error) {
		...
		try {
			count = GenericModel.count("_order = ?", order);
		} catch (Exception e) {
			e.printStackTrace();
