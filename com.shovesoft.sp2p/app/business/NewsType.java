...
//	}
	...
	public static List<NewsType> queryTopTypes(ErrorInfo error) {
		...
		try {
			types = GenericModel.find("parent_id = -1 order by _order").fetch();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "查询类别失败";
	...
	public static List<NewsType> queryChildTypes(long supervisorId, String parentIdStr, ErrorInfo error) {
		...
		try {
			types = GenericModel.find("parent_id = ? order by _order", parentId).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "查询类别失败";
	...
	public static List<NewsType> queryChildTypes(long parentId, ErrorInfo error) {
		...
		try {
			types = GenericModel.find(sql, parentId).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "查询类别失败";
	...
	public static List<v_news_types> queryTypeAndCount(long parentId, ErrorInfo error) {
		...
		try {
			types = GenericModel.find("parent_id = ? and status = true order by _order", parentId).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "查询类别失败";
...
//		}
	...
	public static t_content_news_types queryParentType(long typeId, ErrorInfo error) {
		...
		try {
			parentType = GenericModel.find(sql, typeId).first();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "查询父类别失败";
	...
	public static int hideType(long supvisorId, long typeId, ErrorInfo error) {
		...
		try {
			ids = GenericModel.find(sql2, typeId).fetch();
		} catch(Exception e) {
			e.printStackTrace();
			Logger.info("隐藏类别,更新类别状态时："+e.getMessage());
			error.code = -2;
...
// 		int order = Integer.parseInt(orderStr);
 		...
 		try {
 			count = GenericModel.count("parent_id = ? and _order = ?", typeId, order);
 		} catch(Exception e) {
 			e.printStackTrace();
