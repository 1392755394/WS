...
public class TemplateEmail implements Serializable{
	...
	public void setId(long id) {
		...
		try {
			email = GenericModel.findById(id); 
		} catch(Exception e) {
			e.printStackTrace();
	...
	public static void dealEmailTask() {
		...
		List<t_system_email_sending> sendings = GenericModel.find("").fetch(40);
		...
		if(sendings.size() > 0) {
			JPA.em().createNativeQuery("insert into t_system_email_send (email, title, body) select email, title, body "
					+ " from t_system_email_sending limit 40").executeUpdate();
			JPA.em().createNativeQuery("delete from t_system_email_sending limit 40").executeUpdate();
		}
	}
}
