...
public class CreditLevel implements Serializable{
	...
	public void setId(long id) {
		...
		try {
			creditLevel = GenericModel.findById(id);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private int fillDBE(t_credit_levels cl, ErrorInfo error) {
		...
		try {
			creditList = GenericModel.find(sql).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "数据库异常";
	...
	public int edit(ErrorInfo error) {
		...
		try {
			cl = GenericModel.findById(this.id);
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	private int checkScoreUnique(ErrorInfo error) {
		...
		try {
			list = GenericModel.find("min_credit_score = ?", this.minCreditScore).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "数据库异常";
	...
	public static CreditLevel queryUserCreditLevel(long userId, ErrorInfo error) {
		...
		try {
			cId = GenericModel.find(hql, result).first();
		} catch (Exception e) {
			e.printStackTrace();
		...
		if(null == cId){
			...
			try {
				cId = GenericModel.find(hql).first();
			} catch (Exception e) {
				e.printStackTrace();
				Logger.error("查询用户信用等级,根据排名查询ID:" + e.getMessage());
	...
	public static List<v_credit_levels> queryCreditLevelList(ErrorInfo error) {
		...
		try {
			creditLevels = GenericModel.findAll();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error(e.getMessage());
			error.code = -1;
	...
	public static int compare(long id1, long id2, ErrorInfo error) {
		...
		try {
			cl1 = GenericModel.findById(id1);
			cl2 = GenericModel.findById(id2);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error(e.getMessage());
			error.code = -1;
	...
	public static String queryImageFilename(long id, ErrorInfo error) {
		...
		try {
			...
			return GenericModel.find(hql, id).first();
		} catch (Exception e) {
	...
	public static List<CreditLevel> queryAllCreditLevels(ErrorInfo error){
		...
		try {
			tcreditLevels =  GenericModel.find(hql, Constants.ENABLE).fetch();
		} catch (Exception e) {
	...
	public static List<CreditLevel> queryCreditName(ErrorInfo error) {
		...
		try {
			tcreditLevelc = GenericModel.find(hql, Constants.ENABLE).fetch();
		} catch (Exception e) {
