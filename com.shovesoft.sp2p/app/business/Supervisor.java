...
public class Supervisor implements Serializable{
	...
	public void setId(long id) {
		...
		try {
			supervisor = GenericModel.findById(id);
		} catch (Exception e) {
	...
	public void setName(String name) {
		...
		try {
			li = GenericModel.find("byName", name).fetch();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			this._id = -1;
	...
	public static String checkUkey(String userName, String password, String sign, String keyTime, ErrorInfo info){
		...
		try {
			idList = GenericModel.find(sql, userName2).fetch();
		}catch(Exception e) {
			e.printStackTrace();
			Logger.info("数据库异常");
			info.code = -1;
	...
	public static long queryAdminId(String name, String password ,ErrorInfo error){
		...
		try {
			adminId = GenericModel.find(sql, name, password).first();
		}catch(Exception e) {
			e.printStackTrace();
			Logger.info("数据库异常");
			error.code = -1;
	...
	public boolean haveRight(String action) {
		...
		int rightId = Right.queryRightIdByAction(action, error);
		
		if (error.code < 0) {
			return false;
		}
		
		if (haveRight(rightId)) {
			return true;
		}
	...
	public int edit(ErrorInfo error) {
		...
		try {
			supervisor = GenericModel.findById(this._id);
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
...
//		}
	...
	public static int addCustomers(List<Long> supervisorIds, ErrorInfo error) {
		...
		for (long supervisorId : supervisorIds) {
			...
			try {
				supervisor = GenericModel.findById(supervisorId);
			} catch (Exception e) {
				Logger.error(e.getMessage());
				e.printStackTrace();
				error.code = -1;
	...
	public static PageBean<v_supervisors> queryCustomers(int currPage, int pageSize,
		...
		StringBuffer condition = new StringBuffer("(is_customer = true)");
		List<Object> params = new ArrayList<Object>();
		
		if (lockType != 0) {
			if (1 == lockType) {
				condition.append("and (is_allow_login = true)");
			} else {
				condition.append("and (is_allow_login = false)");
			}
		}
		
		if (StringUtils.isNotBlank(keyword)) {
			condition.append(Constants.QUERY_CUSTOMER_KEYWORD_TYPE[keywordType]);
		...
		try {
			...
			page = GenericModel.find(condition.toString(), params.toArray()).fetch(currPage, pageSize);
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static List<v_supervisors> querySupervisors(List<Long> supervisorIds, ErrorInfo error) {
		...
		StringBuffer sql = new StringBuffer("select s from v_supervisors as s where s.id in (");
		...
		try {
			supervisors = GenericModel.find(sql.toString(), params.toArray()).fetch();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static long queryIdByName(String name, ErrorInfo error) {
		...
		try {
			id_ = GenericModel.find(sql, name).first();
		}catch(Exception e) {
			e.printStackTrace();
			Logger.info("数据库异常");
			error.code = -1;
	...
	public static t_supervisors querySupervisorByName(String name, ErrorInfo error) {
		...
		try {
			supervisor = GenericModel.find("name = ?", name).first();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("数据库异常");
			error.code = -1;
	...
	public static PageBean<v_supervisors> queryCandidateCustomers(int currPage,
		...
		String condition = "(is_customer = false or is_customer = null) and id <> ?";
		List<Object> params = new ArrayList<Object>();
	...
	public static List<t_right_groups> queryGroups(long supervisorId, ErrorInfo error) {
		...
		try {
			list = GenericModel.find(sql, supervisorId).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error(e.getMessage());
			e.printStackTrace();
	...
	public static List<Long> queryAllRightIds(long supervisorId, ErrorInfo error) {
		...
		try {
			list = GenericModel.find(sql, supervisorId, supervisorId).fetch();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static String querySystemSupervisorName() {
		...
		try {
			name = GenericModel.find(sql, SystemSupervisor.ID).first();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
	...
	public static Supervisor currSupervisor() {
		Cookie cookie = Request.current().cookies.get(Constants.COOKIE_KEY_SUPERVISOR_ID);
		...
		Supervisor supervisor = (Supervisor) Cache.get("supervisor_"+encryString);
		
		if(supervisor == null) {
			ErrorInfo error = new ErrorInfo();
			long supervisorId = Security.decryCookie(encryString, error);
	...
	public static void setCurrSupervisor(Supervisor supervisor) {
		String encryString = Security.encryCookie(supervisor.id);
		Response.current().setCookie(Constants.COOKIE_KEY_SUPERVISOR_ID, encryString);
		Cache.set("supervisor_"+encryString, supervisor);
	}
	...
	public static void deleteCurrSupervisor() {
		...
		if(cookie != null) {
			String encryString = cookie.value;
			Cache.delete("supervisor_"+encryString);
		}
		
		Response.current().removeCookie(Constants.COOKIE_KEY_SUPERVISOR_ID);
	}
	...
	public static boolean isEmailExist(String email, ErrorInfo error){
		...
		try {
			supervisor = GenericModel.find("email = ?", email).first();
		} catch(Exception e) {
			e.printStackTrace();
	...
	public boolean isSuperSupervisor(){
		...
		if(Constants.SupervisorLevel.Super == this.level) {
			...
			try {
				obj = GenericModel.find("supervisor_id = ? and group_id = ?", this.id, Constants.SystemSupervisorGroup.ID);
			} catch (Exception e) {
				e.printStackTrace();
				Logger.info(e.getMessage());
