...
public class DBOperation  implements Serializable{
	...
	public static int recoverFromOperation(int operationId, ErrorInfo error) {
		...
		try {
			fileName = GenericModel.find("select filename from t_db_operations where id = ?", (long)operationId).first();
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public static List<v_db_operations> queryOperations(ErrorInfo error) {
		error.clear();
		List<v_db_operations> operations = null;
		
		try {
			operations = GenericModel.findAll();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
