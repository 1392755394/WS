...
public class DealDetail implements Serializable {
	...
	public DealDetail(long userId, int operation, double amount,
			long relationId, double balance, double freeze,
			double recieveAmount, String summary) {
		this.userId = userId;
	 ...
	 * @return
	 */
	public static v_user_for_details queryUserBalance(long userId,
			ErrorInfo error) {
		error.clear();
		...
		try {
			forDetail = GenericModel.findById(userId);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("添加交易记录查询待收金额时：" + e.getMessage());

			error.code = -1;
			error.msg = "添加交易记录查询待收金额时出现异常!";
	 ...
	 * @param error
	 */
	public static void updateUserBalance(long userId, double balance,
			double freeze, ErrorInfo error) {
		error.clear();
	 ...
	 * @param error
	 */
	public static void updateUserBalance2(long userId, double balance,
			double freeze, ErrorInfo error) {
		error.clear();
	 ...
	 *            摘要
	 */
	public static void addPlatformDetail(int operation, long relationId,
			long from, long to, String payment, double amount, int type,
		...
		try {
			balance = ((BigDecimal) JPA.em().createNativeQuery(sql)
	 ...
	 * @param userId
	 * @param type
	 *            信用积分类别 (1 审核资料积分 2 正常还款积分3 成功借款积分4 成功投标积分-1账单逾期扣分) score
	 *            此字段只在type为1时有效
	 * @param relationId
	 *            当类别为：1 t_user_audit_items中的id,2 账单的id, 3 借款标的id, 4
	 *            t_invest理财id -1 逾期账单id
	 * @param summary
	 *            摘要说明
	 * @param error
	 */
	public static int addCreditScore(long userId, int type, int score,
			long relationId, String summary, ErrorInfo error) {
		error.clear();
		...
		String sql = "update t_users set credit_score = credit_score + ? where id = ?";

		int rows = 0;

		try {
			creditScore.save();
			rows = JpaHelper.execute(sql, creditTypeScore, userId)
					.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @param error
	 */
	public static int addScore(long userId, int type, double amount,
			long relationId, String summary, ErrorInfo error) {
		error.clear();
		BackstageSet backstageSet = BackstageSet.getCurrentBackstageSet();
		...
		int score = backstageSet.moneyToSystemScore * money;
	 ...
	 * @param error
	 */
	public static void userEvent(long userId, int type, String descrption,
			ErrorInfo error) {
		error.clear();
	 ...
	 * @param error
	 */
	public static void supervisorEvent(long supervisorId, int type,
			String descrption, ErrorInfo error) {
		error.clear();
	 ...
	 * @return
	 */
	public static PageBean<v_supervisor_events> querySupervisorEvents(
			int currPage, int pageSize, int keywordType, String keyword,
			Date beginTime, Date endTime, ErrorInfo error) {
		error.clear();
		...
		try {
			minDate = GenericModel.find(
					"SELECT MIN(time) from v_supervisor_events").first();
			count = (int) GenericModel.count(condition.toString(),
					params.toArray());
			page = GenericModel.find(condition.toString(),
					params.toArray()).fetch(currPage, pageSize);
		} catch (Exception e) {
		...
		map.put("days",
				(minDate == null) ? 0 : DateUtil.daysBetween(minDate,
						new Date()));
	 ...
	 * @return
	 */
	public static PageBean<v_supervisor_events> querySupervisorDeleteEvents(
			int currPage, int pageSize, ErrorInfo error) {
		error.clear();
		...
		try {
			count = (int) GenericModel.count("type_id = ?",
					SupervisorEvent.DELETE_EVENT);
			page = GenericModel.find("type_id = ?",
					SupervisorEvent.DELETE_EVENT).fetch(currPage, pageSize);
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static int deleteEvents(int type, ErrorInfo error) {
		...
		supervisorEvent(Supervisor.currSupervisor().id,
				SupervisorEvent.DELETE_EVENT, description, error);
	...
	public static void addCapital(double amount, String summary, ErrorInfo error) {
		...
		DealDetail.addPlatformDetail(DealType.ADD_CAPITAL, supervisorId, -1,
				-1, DealType.HAND, amount, 1, summary, error);
		...
		DealDetail.supervisorEvent(supervisorId, SupervisorEvent.ADD_MONEY,
				"添加保障金", error);
	 ...
	 * @return
	 */
	public static PageBean<v_platform_detail> platformDetail(int type,
			int operation, int side, Date beginTime, Date endTime, String name,
		...
		conditions.append(Constants.TYPE[type]);

		if (operation != 0) {
			conditions.append("and operation = ? ");
			values.add(operation);
		}

		if (StringUtils.isNotBlank(name)) {
			conditions.append(Constants.SIDE[side]);
	 ...
	 * @return
	 */
	public static List<t_platform_detail_types> queryType(ErrorInfo error) {
		error.clear();

		List<t_platform_detail_types> detialTypes = null;

		try {
			detialTypes = GenericModel.findAll();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static Map<String, Double> queryUserFund(long userId, ErrorInfo error) {
		...
		try {
			user_amount = GenericModel.find(
					"select balance from t_users where id = ?", userId).first();
		...
		try {
			user_amount2 = GenericModel.find(
					"select balance2 from t_users where id = ?", userId)
					.first();
		} catch (Exception e) {
			Logger.error("交易记录->及时查询用户的可用余额：" + e.getMessage());
			error.code = -1;
		...
		try {
			freeze = GenericModel.find("select freeze from t_users where id = ?",
					userId).first();
		} catch (Exception e) {
	 ...
	 *            信息值
	 */
	public static void addSystemScore(long userId, double investAmount,
	...
	public static PageBean<t_user_recharge_details> queryUserRechargeDetails(
			int type, int status, String name, Date startDate, Date endDate,
			int currPage) {
		...
		String sql = "select count(1) from t_user_recharge_details urd JOIN t_users u ON urd.user_id = u.id"
				+ conditions.toString();
		...
		sql = "select urd.* from t_user_recharge_details urd JOIN t_users u ON urd.user_id = u.id "
				+ conditions.toString() + " order by time desc limit ?, ?";
		...
		query.setParameter(size + 1, (currPage - 1) * Constants.PAGE_SIZE);
		query.setParameter(size + 2, Constants.PAGE_SIZE);
