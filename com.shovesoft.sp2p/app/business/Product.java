...
public class Product implements Serializable{
	...
	public String loanAudit_filename; // 平台上传资料图片
	...
	public void setId(long id) {
		...
		try {
			product = GenericModel.findById(id);
		} catch (Exception e) {
	 ...
	 * @return List<Product>
	 */
	public static List<Product> queryProduct(ErrorInfo error) {
		error.clear();
		...
		try {
			tpoducts = GenericModel.find(hql, Constants.ENABLE, Constants.NOT_IS_AGENCY, Constants.PC, Constants.PC_APP).fetch(8);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<Product> queryProduct(PageBean<Product> pageBean, String name, ErrorInfo error) {
		...
		try {
			tproducts = GenericModel.find(hql).fetch(pageBean.currPage, pageBean.pageSize);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static Product queryAgencyProduct(ErrorInfo error){
		...
		try {
			id = GenericModel.find(hql, Constants.ENABLE).first();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean checkName(String name, long id){
		...
		try {
			name = GenericModel.find(hql, name.trim()).first();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static int queryLoanType(long pid) {
		...
		try {
			loanType = GenericModel.find(hql, pid).first();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<Integer> queryLoanType(ErrorInfo error){
		...
		try {	
			return GenericModel.find(hql, Constants.NET_VALUE_BID, Constants.S_REPAYMENT_BID).fetch();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static double queryNetValueBailScale(ErrorInfo error) {
		...
		try {
			bailScale = GenericModel.find(hql, Constants.NET_VALUE_BID).first();
		} catch (Exception e) {
	...
	public static List<Product> queryProductNames(boolean flag, ErrorInfo error){
		...
		try {
			tproducts = GenericModel.find(hql, Constants.ENABLE).fetch();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private int addOrEdit(t_products product) {
		...
		product.is_deal_password = this.isDealPassword; // 是否需要交易密码
	...
	public int addProductLableAndFiled(long productId, ErrorInfo error){
		...
		/* 得到集合 */
        List<Map<String,Object>> lables = JSONArray.fromObject(this.jsonLables);
