...
public class TemplateSms implements Serializable{
	...
	public static void dealSmsTask() {
		List<t_system_mobile_sms_sending> sendings = GenericModel.find("").fetch(100);
		...
		for(t_system_mobile_sms_sending sending : sendings) {
			...
			try {
				DbMsm.send(account, password, sending.body, sending.mobile);
			} catch (Exception e) {
				e.printStackTrace();
		...
		if(sendings.size() > 0) {
			JPA.em().createNativeQuery("insert into t_system_mobile_sms_send (mobile, body) select mobile, body "
					+ " from t_system_mobile_sms_sending limit 100").executeUpdate();
			
			JPA.em().createNativeQuery("delete from t_system_mobile_sms_sending limit 100").executeUpdate();
		}
	}
}
