...
public class BottomLinks  implements Serializable{
	...
	public void setId(long id) {
		...
		t_content_advertisements_links bottominfo = GenericModel
				.findById(id);
	...
	public static Map<String, List<BottomLinks>> queryBottomLinks() {
		...
		try {
			links = GenericModel.find(sql).fetch();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<t_content_advertisements_links> queryBottomLinksByKey(
		...
		try {
			bottomLinks = GenericModel.find("_key = ? order by _order", key).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("查询底部链接时：" + e.getMessage());
	...
	public List<BottomLinks> queryAllBottomLink(ErrorInfo error) {
		...
		try {
			BottomLinkList = GenericModel.find(sql, this.key)
					.fetch();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("查询底部链接时：" + e.getMessage());
