...
public class TemplateStation extends Template  implements Serializable{
	...
	public void setId(long id) {
		...
		try {
			station = GenericModel.findById(id);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String getTitle() {
		if (null == title && this.id > 0) {
			try {
				...
				this.title = GenericModel.find(sql, this.id).first();
			} catch (Exception e) {
				e.printStackTrace();
	...
	public String getContent() {
		if (null == content && this.id > 0) {
			...
			try {
				this.content = GenericModel.find(sql, this.id).first();
			} catch (Exception e) {
				e.printStackTrace();
				Logger.info("查询站内信模板标题时" + e.getMessage());
			}
	...
	public static List<TemplateStation> queryScenarios(ErrorInfo error) {
		...
		try {
			templates = GenericModel.find(sql).fetch();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void dealStationTask() {
		...
		List<t_system_message_sending> sendings = GenericModel.find("").fetch(100);
		...
		if(sendings.size() > 0) {
			//JPA.em().createNativeQuery("insert into t_messages (sender_supervisor_id, title, content, receiver_user_id) "
			//		+ " select 1, title, body, receiver_user_id from t_system_message_sending limit 100").executeUpdate();
			
			JPA.em().createNativeQuery("delete from t_system_message_sending limit 100").executeUpdate();
		}
	}
