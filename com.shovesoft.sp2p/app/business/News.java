...
public class News implements Serializable {
	...
	public int showType;
	/**
	 * 0 不推荐 homepage 1 推荐至首页 marquee 2 荐跑马灯效果区 top 3 推荐首页头条
	 */
	...
	public int updateNews(long supervisorId, long adsId, ErrorInfo error) {
		...
		try {
			news = GenericModel.findById(adsId);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	public static int deleteNews(long supervisorId, String adsIdStr,
			ErrorInfo error) {
		error.clear();
	 ...
	 * @return
	 */
	public static int updateNewsUse(long supervisorId, String adsIdStr,
			String isUseStr, ErrorInfo error) {
		error.clear();
	 ...
	 */
	public static boolean orderExist(String typeIdStr, String orderStr,
			ErrorInfo error) {
		...
		try {
			count = GenericModel.count("type_id = ? and _order = ?", typeId,
					order);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("更新新闻使用状态时：" + e.getMessage());
	 ...
	 * @return
	 */
	public static PageBean<News> queryNewsBySupervisor(String topTypes,
			String typeIdStr, String title, String orderTypeStr,
			String orderStatus, String currPageStr, String pageSizeStr,
			ErrorInfo error) {
		error.clear();
		...
		if (typeId != 0) {
			...
			List<Long> typeList = GenericModel.find(
					"select id from t_content_news_types where parent_id = ?",
					topTypeId).fetch();
		...
		try {
			count = (int) GenericModel.count(conditions.toString(),
					values.toArray());
			contents = GenericModel.find(conditions.toString(),
	 ...
	 * @return
	 */
	public static List<t_content_news> queryNewForFront(long type_id, int size,
			ErrorInfo error) {
		error.clear();
		...
		String sql = "select new t_content_news(id, title,content, read_count,image_filename,time) from t_content_news where "
				...
				+ " order by time desc, _order";

		List<t_content_news> news = new ArrayList<t_content_news>();

		try {
			news = GenericModel.find(sql, type_id, new Date()).fetch(1, size);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<t_content_news> queryNewForFrontHome(ErrorInfo error) {
		...
		try {
			news = GenericModel.find(sql, new Date()).fetch(1, 4);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("财富资讯查询时：" + e.getMessage());
	...
	public static List<t_content_news> queryNewForFrontHeadlines(ErrorInfo error) {
		...
		try {
			news = GenericModel.find(sql, new Date()).fetch(1, 2);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("财富资讯查询时：" + e.getMessage());
	...
	public static List<t_content_news> queryNewForFrontMarquee(ErrorInfo error) {
		...
		String sql = "select new t_content_news(id, title) from t_content_news where "
				+ "location_pc in(2,3,6,7) and start_show_time <= ? and is_use = true order by _order";

		List<t_content_news> news = new ArrayList<t_content_news>();

		try {
			news = GenericModel.find(sql, new Date()).fetch(1, 5);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("财富资讯查询时：" + e.getMessage());
	 ...
	 * @return
	 */
	public static PageBean<t_content_news> queryNewsByTypeId(String typIdStr,
			String currPageStr, String pageSizeStr, String keyword,
			ErrorInfo error) {
		...
		String sqlPage = "select new t_content_news(id, time, title, content, read_count) from "
				+ "t_content_news news where news.type_id = ? and news.is_use = true and "
				+ "news.start_show_time <= ? "
				+ searchCondition
				+ " order by news._order,time desc";

		try {
			count = (int) GenericModel.count(sqlCount, params.toArray());
			news = GenericModel.find(sqlPage, params.toArray()).fetch(
					currPage, pageSize);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	public static List<News> queryNewsDetail(String idStr, String keyword,
			ErrorInfo error) {
		long id = 0;
		...
		try {
			newsLast = GenericModel.find(sqlLast, params.toArray()).first();
			newsNext = GenericModel.find(sqlNext, params.toArray()).first();
		...
		JpaHelper.execute(
				"update t_content_news set read_count = read_count+1"
	...
	public static int support(long newsId, ErrorInfo error) {
		...
		Query queryUpdate = JPA.em().createQuery(sqlUpdate)
		...
		try {
			...
			support = GenericModel.find(
					"select support from t_content_news where id = ?", newsId)
					.first();
		} catch (Exception e) {
			JPA.setRollbackOnly();
	...
	public static int opposition(long newsId, ErrorInfo error) {
		...
		try {
			rows = queryUpdate.executeUpdate();
			opposition = GenericModel.find(
					"select opposition from t_content_news where id = ?",
					newsId).first();
		} catch (Exception e) {
			JPA.setRollbackOnly();
			Logger.error(e.getMessage());
	...
	public static String queryContent(long newsId, ErrorInfo error) {
		...
		try {
			return GenericModel.find(hql, newsId).first();
		} catch (Exception e) {
	 ...
	 * @return 服务条款
	 */
	public static String queryContentByTypeId(long newsId, ErrorInfo error) {
		error.clear();
		if (newsId == 19) {
		...
		String hql = "select content from t_content_news where type_id = ? ";
	...
	public static List<t_content_news> hire(long type_id, ErrorInfo error) {

		String hql = "from t_content_news where type_id = ? order by time desc";

		List<t_content_news> bids = null;

		if (15 < type_id && type_id != 19) {
			return null;
		}
		try {

			bids = GenericModel.find(hql, type_id).fetch(10);
	 ...
	 * @return
	 */
	public static t_content_news getPrincipalGuaranteeNews(long typeId,
			ErrorInfo error) {

		t_content_news temp = new t_content_news();

		try {
			temp = GenericModel.find("type_id = ?  order by id desc", typeId)
					.first();
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
	...
	public static PageBean<t_content_advertisements_partner> queryPartners(
		...
		try {
			...
			page = GenericModel.find("").fetch(currPage,
					pageSize);
		} catch (Exception e) {
	...
	public static Object[] queryInvestDataSum() {
		...
		try {
			count = GenericModel.count("master_identity in (?, ?)",
					Constants.INVEST_USER, Constants.LOAN_INVEST_USER);
		} catch (Exception e) {
		...
		try {
			sum = GenericModel
					.find("select sum(bill.overdue_fine+bill.repayment_corpus+bill.repayment_interest) from t_bills bill")
					.first();
		} catch (Exception e) {
		...
		double sumBill = Convert.strToDouble(sum + "", 0) + 127000000;
		datas[1] = Arith.round(sumBill, 4);
		datas[2] = Arith.round(sumBill * 0.16, 2);

		//int lots = 0;
	///	try {
	//		lots = t_bids
	//				.find("select count(*) from t_bids group by status > ?" ,"2").first();
	//	} catch (Exception e) {
			// TODO Auto-generated catch block
	//		e.printStackTrace();
	//	}
	//	System.out.print(lots);
	//	datas[3] = Arith.round(sumBill * 0.065, 2);
	...
	public static String queryVipAgreement() {
		...
		String agreement = GenericModel.find(sql, -1009L).first();
	...
	public static PageBean<News> queryNewsByTypeIdApp(String currPageStr,
		...
		try {
			count = (int) GenericModel.count(sqlCount, new Date());
			news = GenericModel.find(sqlPage, new Date()).fetch(currPage,
					pageSize);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("查询财富资讯列表情况时：" + e.getMessage());
