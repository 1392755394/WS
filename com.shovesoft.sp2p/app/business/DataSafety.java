...
public class DataSafety implements Serializable {
	...
	public void setId(long id) {
		...
		try {
			userMap = GenericModel.find(sql, id).first();
		} catch (Exception e) {
			e.printStackTrace();
		...
		String sql2 = "select new Map(a.amount as amount, a.balance as balance, a.freeze as freeze,"
				+ " a.recieve_amount as recieveAmount) from t_user_details as a where a.user_id = ? order by id desc limit 1";
		try {
			userDetailMap = GenericModel.find(sql2, id).first();
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (userMap != null) {
			this.balance1 = String.valueOf(userMap.get("balance"));
			this.freeze1 = String.valueOf(userMap.get("freeze"));
			this.sign1 = (String) userMap.get("sign1");
			this.sign2 = (String) userMap.get("sign2");
		}

		if (userDetailMap != null) {
			this.balance2 = String.valueOf(userDetailMap
					.get("balance"));
			this.freeze2 = String.valueOf(userDetailMap.get("freeze"));
			this.recieveAmount = String.valueOf(userDetailMap
					.get("recieveAmount"));
			this.amount = String.valueOf(userDetailMap.get("amount"));
		} else {
			this.balance2 = 0.00 + "";
			this.freeze2 = 0.00 + "";
	...
	public boolean signCheck(ErrorInfo error) {
		...
		String userSign1 = com.shove.security.Encrypt.MD5(Long
				.toString(this._id)
				+ this.balance1
				+ this.freeze1
				+ Constants.ENCRYPTION_KEY);

		String userSign2 = com.shove.security.Encrypt.MD5(Long
				.toString(this._id)
				+ this.balance2
				+ this.freeze2
				+ this.amount + this.recieveAmount + Constants.ENCRYPTION_KEY);

		if (!this.sign1.equalsIgnoreCase(userSign1)) {
			error.code = -1;
			error.msg = "尊敬的用户，你的账户资金出现异常变动，请速联系管理员";			
			Logger.error("账户资金1出现异常变动" + "userId:" + this._id + "|userSinOne:"
					+ userSign1 + "|userSinTwo:" + userSign2);
			return false;
		}
		...
		if (!this.sign2.equalsIgnoreCase(userSign2)) {
			...
			Logger.error("账户资金2出现异常变动" + "userId:" + this._id + "|userSinOne:"
					+ userSign1 + "|userSinTwo:" + userSign2);
			return false;
		}
	...
	public int updateSign(ErrorInfo error) {
		...
		Query query = em.createQuery(updateSql).setParameter(1, userSign1)
				.setParameter(2, userSign2).setParameter(3, this._id);
