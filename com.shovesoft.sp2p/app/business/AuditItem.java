...
public class AuditItem implements Serializable{
	...
	public static String queryMark(long id){
		try {
			return GenericModel.find("select mark from t_dict_audit_items where id = ? ", id).first();
		} catch (Exception e) {
	...
	public void setMark(String mark) {
		...
		try {
			item = GenericModel.find("mark = ?", mark).first();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static PageBean<AuditItem> queryAuditItems(String currPage, String pageSize, String keyword, boolean flag, ErrorInfo error) {
		...
		try {
			tauditItems = GenericModel.find(conditions.toString(), values.toArray()).fetch(pageBean.currPage, pageBean.pageSize);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<AuditItem> queryEnableAuditItems(ErrorInfo error) {
		...
		try {
			tauditItems = GenericModel.find("is_use", Constants.ENABLE).fetch();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static PageBean<t_dict_audit_items> queryEnableAuditItems(String key, int currPage, int pageSize, ErrorInfo error) {
		...
		try {
			page.totalCount =  (int) GenericModel.count(conditions.toString(), values.toArray());
			tauditItems = GenericModel.find(conditions.toString(), values.toArray()).fetch(page.currPage, page.pageSize);
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("资料->查询有效的审计资料列表:" + e.getMessage());
			error.msg = error.FRIEND_INFO + "查询审计资料列表有误!";
	...
	public static List<AuditItem> queryAuditItems(ErrorInfo error){
		...
		try {
			tauditItems = GenericModel.find(hql, Constants.ENABLE).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("资料->查询有效的审计资料列表:" + e.getMessage());
			error.msg = error.FRIEND_INFO + "查询审计资料列表有误!";
	...
	public static long auditItemCount() {
		try{
			return GenericModel.count("is_use = ?", Constants.ENABLE);
		}catch(Exception e) {
			e.printStackTrace();
	...
	public void edit(long id, ErrorInfo error) {
		...
		try {
			auditItem = GenericModel.findById(id);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static double queryItemFeeById(long itemId, ErrorInfo error) {
		...
		try {
			fee = GenericModel.find(hql, itemId).first();
		} catch (Exception e) {
			error.code = -4;
			error.msg = "查询失败!";
