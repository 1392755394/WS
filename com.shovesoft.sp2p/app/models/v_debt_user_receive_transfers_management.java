...
public class v_debt_user_receive_transfers_management extends Model {
	...
	public Double getReceiving_amount_success() {
		...
		Long investId = 0l;
		try {
			investId = GenericModel.find("select id from t_invests where transfers_id = ? ", this.transer_id).first();
		} catch (Exception e) {
			e.printStackTrace();
			investId = 0l;
		}
	...
	public Double getHas_received_amount_success() {
		...
		if(investId != null && investId > 0){
			try {
				temp = GenericModel.find(" select sum(receive_corpus + receive_interest + overdue_fine) from t_bill_invests where invest_id = ? and status in (-3,-4,0)", investId).first();
			} catch (Exception e) {
				e.printStackTrace();
				temp = 0.0;
			}
