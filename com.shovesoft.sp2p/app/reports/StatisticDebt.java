...
public class StatisticDebt {
	 ...
	 public  static  long queryDebtAccount(){
		 ...
		 try {
			 debt_account = GenericModel.find(sql).first() ;//债权转让标总数量
		} catch (Exception e) {
			e.printStackTrace();
			debt_account = 0;
		}
	 ...
	 public static Double queryDebtAmounSum(){
		 ...
		 try {
			 debt_amount_sum = GenericModel.find(sql).first() ;//债权转让总金额 
		} catch (Exception e) {
			e.printStackTrace();
			debt_amount_sum = 0.0;
		}
	 ...
	 public static long queryIncreaseDebtAccount(){
		...
		try {
			increase_debt_account = GenericModel.find(sql).first();// 本月新增转让标数量
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 public static Double queryIncreaseDebtAmountSum(){
		...
		try {
			increase_debt_amount_sum = GenericModel.find(sql).first();// 本月新增转让总额
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 public static Long queryHasOverdueDebt(){
		...
		try {
			investIds = GenericModel.find(sql).fetch();
		} catch (Exception e) {
			e.printStackTrace();
			has_overdue_debt = 0l;
		}
		...
		if(investIds.size() > 0){
			...
			try {
				bidIds = GenericModel.find(sql).fetch();
			} catch (Exception e) {
				e.printStackTrace();
				has_overdue_debt = 0l;
			}
			...
			if(bidIds.size() > 0){
				...
				try {
					has_overdue_debt = GenericModel.find(sql).first();
				} catch (Exception e) {
					e.printStackTrace();
					has_overdue_debt = 0l;
				}
	 ...
	 public static long querySuccessDebtAmount(){
		...
		try {
			success_debt_amount = GenericModel.find(sql).first();// 债权转让成功标数量
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 public static boolean judgeIsNew(int year,int month){
		 ...
		 try {
			 statistic = GenericModel.find("  year = ? and month = ?",year,month).first();
		} catch (Exception e) {
			e.printStackTrace();
		}
