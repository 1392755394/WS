...
public class StatisticProduct {
	...
	private static int add(t_products product, ErrorInfo error) {
		...
		entity.overdue_per = entity.released_bids_num==0 ? 0 : entity.overdue_num / entity.released_bids_num;
		entity.bad_bids_num = queryBadBidsNum(productId, error);
		entity.bad_bids_per = entity.released_bids_num==0 ? 0 : entity.bad_bids_num / entity.released_bids_num;
	...
	private static int update(t_products product, ErrorInfo error) {
		...
		try {
			entity = GenericModel.find("product_id = ? and year = ? and month = ?", productId, year, month).first();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	private static boolean isAdd(int productId, ErrorInfo error) {
		...
		try {
			count = (int)GenericModel.count("product_id = ? and year = ? and month = ?", productId, year, month);
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static int queryReleasedBidsTotalNum(ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and `status` in (4, 5)";
	...
	public static int queryReleasedBidsNum(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and `status` in (4, 5)";
		Query query = JPA.em().createNativeQuery(sql);
	...
	public static double queryReleasedAmount(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select sum(amount) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and `status` in (4, 5)";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static double queryAverageBidAmount(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select avg(amount) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and `status` in (4, 5)";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static int queryOverdueNum(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and id in (select bid_id from t_bills where overdue_mark <> 0)";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static int queryBadBidsNum(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and id in (select bid_id from t_bills where overdue_mark = -3)";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static int queryBidsNum(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m')";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static int queryInvestUserNum(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select count(distinct user_id) from t_invests where bid_id in (select id from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m'))";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static double queryAverageAnnualRate(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select avg(apr) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m')";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static int querySuccessBidsNum(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and `status` = 5";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
	...
	public static double querySuccessBidsAmount(int productId, ErrorInfo error) {
		error.clear();
		String sql = "select sum(amount) from t_bids where product_id = ? and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and `status` = 5";
		Query query = JPA.em().createNativeQuery(sql);
		query.setParameter(1, productId);
		Object obj = null;
