...
public class StatisticBorrow {
	...
	private static int update(ErrorInfo error) {
		...
		try {
			entity = GenericModel.find("year = ? and month = ?", year, month).first();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	private static boolean isAdd(ErrorInfo error) {
		...
		try {
			count = (int)GenericModel.count("year = ? and month = ?", year, month);
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static double queryTotalBorrowAmount(ErrorInfo error) {
		error.clear();
		String sql = "select sum(amount) from t_bids";
	...
	public static double queryThisMonthBorrowAmount(ErrorInfo error) {
		error.clear();
		String sql = "select sum(`t_bids`.`amount`) AS `sum(amount)` from `t_bids` where (date_format(`t_bids`.`time`,'%Y%m') = date_format(curdate(),'%Y%m'))";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static int queryTotalBorrowUserNum(ErrorInfo error) {
		...
		try {
			count = (int) GenericModel.count("master_identity = 1 or master_identity = 3");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static int queryNewBorrowUserNum(ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_users where date_format(master_time_loan,'%Y%m') = date_format(curdate(),'%Y%m') or date_format(master_time_complex,'%Y%m') = date_format(curdate(),'%Y%m')";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static double queryRepayingBorrowAmount(ErrorInfo error) {
		error.clear();
		String sql = "select sum(amount) from t_bids where status = 4 and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m')";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static int queryReleasedBidsNum(ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where status = 4 and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m')";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static double queryAverageAnnualRate(ErrorInfo error) {
		error.clear();
		String sql = "select avg(apr) from t_bids where status = 4 and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m')";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static double queryAverageBorrowAmount(ErrorInfo error) {
		error.clear();
		String sql = "select avg(amount) from t_bids where status = 4 and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m')";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static int queryOverduedBidsNum(ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where status = 4 and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and id in (select bid_id from t_bills where overdue_mark <> 0)";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static double queryOverdueAmount(ErrorInfo error) {
		error.clear();
		String sql = "select sum(repayment_corpus+real_repayment_interest+overdue_fine) from t_bills where overdue_mark != 0 and (date_format(mark_overdue_time,'%Y%m') = date_format(curdate(),'%Y%m'))";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static int queryBadBidsNum(ErrorInfo error) {
		error.clear();
		String sql = "select count(*) from t_bids where status = 4 and date_format(time,'%Y%m') = date_format(curdate(),'%Y%m') and id in (select bid_id from t_bills where overdue_mark = -3)";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
	...
	public static double queryBadBillAmount(ErrorInfo error) {
		error.clear();
		String sql = "select sum(repayment_corpus+real_repayment_interest+overdue_fine) from t_bills where overdue_mark = -3 and (date_format(mark_bad_time,'%Y%m') = date_format(curdate(),'%Y%m'))";
		Query query = JPA.em().createNativeQuery(sql);
		Object obj = null;
