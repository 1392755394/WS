...
public class StatisticPlatformIncome {
	 ...
	 public static Double queryLoanManagefee(){
		...
		try {
			loan_manage_fee = GenericModel.find(sql,
	 ...
	 public static Double queryRechargeManagefee(){
		 ...
		 String sql = "select sum(amount) from t_user_details where operation = ?  and DATE_FORMAT(time, '%Y%m%d') = DATE_FORMAT(CURDATE(),'%Y%m%d')";
		 try {
			 recharge_manage_fee = GenericModel.find(sql, DealType.CHARGE_RECHARGE_FEE).first();//充值手续费
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 public static Double queryWithdrawManagefee(){
		 ...
		 String sql = "select sum(amount) from t_user_details where operation = ?  and DATE_FORMAT(time, '%Y%m%d') = DATE_FORMAT(CURDATE(),'%Y%m%d')";
		 try {
			 withdraw_manage_fee = GenericModel.find(sql, DealType.CHARGE_WITHDRAWALT).first();//提现手续费
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 public static Double queryVipManagefee(){
		 ...
		 try {
			 vip_manage_fee = GenericModel.find(sql, DealType.CHARGE_VIP).first();//VIP会员费
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 public static Double queryInvestManagefee(){
		 ...
		 try {
			 invest_manage_fee = GenericModel.find(sql, DealType.CHARGE_INVEST_FEE).first();//理财管理费
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 public static Double queryDebtTransferManagefee(){
		 ...
		 try {
			 debt_transfer_manage_fee = GenericModel.find(sql, DealType.CHARGE_DEBT_TRANSFER_MANAGEFEE).first();//债权转让管理费
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 public static Double queryItemauditManagefee(){
		 ...
		 String sql = "select sum(amount) from t_user_details where operation = ?  and DATE_FORMAT(time, '%Y%m%d') = DATE_FORMAT(CURDATE(),'%Y%m%d')";
		 try {
			 item_audit_manage_fee = GenericModel.find(sql, DealType.CHARGE_AUDIT_ITEM).first();//资料审核费
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 public static Double queryOverdueManagefee(){
		...
		try {
			overdue_manage_fee = GenericModel.find(sql,
	...
	public static boolean judgeIsnew(int year,int month,int day){
		...
		try {
			 incom = GenericModel.find(" year = ? and month = ? and day = ? ", year,month,day).first();
		} catch (Exception e) {
			e.printStackTrace();
		}
