...
public class StatisticInvest {
	...
	public static Double queryInvestAccoumt(){
		...
		String sql = "select sum(amount) from t_invests  where transfer_status = 0";
		
		try {
			invest_accoumt = GenericModel.find(sql).first();//累计理财投标总额
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Double queryIncreaseInvestAccount(){
		...
		String sql =  "select sum(amount) from t_invests where DATE_FORMAT(time, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m') and transfer_status = 0 ";
		try {
			increase_invest_account = GenericModel.find(sql).first();//本月新增理财总额
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static long queryInvestUseraccount(){
		...
		try {
			invest_user_account = GenericModel.find(sql).first();// 累计理财会员数
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static long queryIncreaseInvestUseraccount(){
		...
		String sql = "select count(*) from t_users where master_identity in (2,3) and DATE_FORMAT(master_time_invest, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m')";

		try {
			increase_invest_user_account = GenericModel.find(sql).first();// 新增理财会员数
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Double queryIncreaseInvestUserblance(){
		...
		String sql = "select sum(balance) from t_users where master_identity in (2,3) and DATE_FORMAT(master_time_invest, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m')";
		
		if(Constants.IPS_ENABLE) {
			sql = "select sum(balance + balance2) from t_users where master_identity in (2,3) and DATE_FORMAT(master_time_invest, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m')";
		} 
		
		try {
			temp = GenericModel.find(sql).first();//新增理财会员总余额
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static long queryAllUser(){
		...
		try {
			temp = GenericModel.find(sql).first();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Double queryBidCount(){
		...
		try {
			temp = GenericModel.find(sql).first();// 借款标总额
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static long queryInvestCount(){
		...
		try {
			investCount = GenericModel.find(sql).first();// 投标数
		} catch (Exception e) {
			e.printStackTrace();
		}
		return investCount;
	...
	public static long queryDebtAccount(){
		...
		try {
			debtAccount = GenericModel.find(sql).first();// 该月所有投资的债权标总和
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static long queryDebtNum(){
		...
		try {
			debtNum = GenericModel.find(sql).first();//该月 所有投资债权标的 人数 总和
		} catch (Exception e) {
			e.printStackTrace();
		}
		return debtNum;
	...
	public static boolean judgeIsnew(int year,int month){
		...
		try {
			invest = GenericModel.find(" year = ? and month = ?", year,month).first();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void investSituationStatistic(){
		...
		double per_capita_invest_amount = 0;//人均理财金额
		double per_bid_average_invest_amount = 0;//每标人均投标金额
		double per_capita_invest_debt = 0;//人均投资债权标数量
		double per_capita_balance = 0;//新增人均账户余额
		double invest_user_conversion = 0;//理财会员转化率
		
		if(increase_invest_user_account > 0){
			per_capita_invest_amount = Arith.div(increase_invest_account, increase_invest_user_account, 2);//人均理财金额
		}
		...
		if(t > 0){
			invest_user_conversion = Arith.div(increase_invest_user_account, t, 4)*100;//理财会员转化率
		}else{
