...
public class EmailUtil {
	...
	public static int emailFindUserName(String email, ErrorInfo info) {
		...
		try{
			name = GenericModel.find(sql, email).first();
			values = GenericModel.find(optionSql, OptionKeys.PLATFORM_TELEPHONE, OptionKeys.PLATFORM_NAME).fetch();
			template = GenericModel.find("scenarios = ?", Constants.FIND_USERNAME).first();
		} catch(Exception e) {
			e.printStackTrace();
	...
	public static int emailFindUserPassword(String userName, String email, ErrorInfo info) {
		...
		try{
			values = GenericModel.find(optionSql, OptionKeys.PLATFORM_TELEPHONE, OptionKeys.PLATFORM_NAME).fetch();
			template = GenericModel.find("scenarios = ?", Constants.RESET_PASSWORD).first();
		} catch(Exception e) {
			e.printStackTrace();
			info.code = -1;
			info.msg = "系统参数或邮件模板查询出现错误";
	...
	public static int emailResetSecretQuestion(String userName, String email, ErrorInfo info) {
		...
		try{
			values = GenericModel.find(optionSql, OptionKeys.PLATFORM_TELEPHONE, OptionKeys.PLATFORM_NAME).fetch();
			template = GenericModel.find("scenarios = ?", Constants.RESET_SECRET_QUESTION).first();
		} catch(Exception e) {
			e.printStackTrace();
