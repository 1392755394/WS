...
public class Security {
	...
	public static String encryCookie(long id) {
		return Encrypt.encrypt3DES(id+"", Constants.ENCRYPTION_KEY); 
	}
	
	public static long decryCookie(String encryCookie, ErrorInfo error) {
		error.clear();
		...
		encryCookie = Encrypt.decrypt3DES(encryCookie, Constants.ENCRYPTION_KEY);
		...
		if(!NumberUtil.isNumericInt(encryCookie)) {
			error.code = -1;
			error.msg = "cookie信息有误";
		...
		error.code = 1;
		return Long.parseLong(encryCookie);
	}
